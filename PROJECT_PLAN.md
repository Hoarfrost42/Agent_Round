...
| 2026-01-11 | Server | **SSE 流式支持**：`/api/sessions/{id}/stream` 已实现并按规范输出事件，当前为 token 分块模拟；前端需对接 `EventSource`。 | **已完成** |
...
| 优先级 | 问题 | 位置 | 描述 | 状态 |
|--------|------|------|------|------|
| ⚠️ 中 | `shutdown` 使用强制退出 | `main.py:76` | `os._exit(0)` 可能导致数据未保存，建议优雅关闭 | ✅ |
| ℹ️ 低 | 废弃的内存存储文件 | `backend/core/session.py` | 已被 SQLite 版替代，建议删除或标记废弃 | ✅ |
| ℹ️ 低 | 空壳 Provider 实现 | `anthropic_provider.py` / `google_provider.py` | 仅有空实现，需补充或标注未实现 | ✅ |
| ℹ️ 低 | ContextBuilder 重复读取 | `sessions.py:247-250` | 每次 SSE 循环都重新构建，可缓存优化 | ✅ |
