...
| 优先级 | 建议 | 描述 | 工作量 |
|--------|------|------|--------|
| 🔴 高 | **决策卡片交互** | 每轮结束显示"认可共识/继续讨论"按钮 | 小 |
| ⚠️ 中 | **Markdown 渲染增强** | 代码高亮、表格、数学公式（KaTeX） | 小 |
| ⚠️ 中 | **消息编辑/重发** | 编辑已发送消息并重新生成 | 中 |
| ⚠️ 中 | **模型回复折叠** | 长回复折叠，点击展开 | 小 |
| ⚠️ 中 | **会话导出** | 导出为 Markdown/PDF/JSON | 小 |
| ⚠️ 中 | **预设模板** | "系统分析""代码审查"等场景 | 小 |
| ℹ️ 低 | **暗色主题** | 深色模式切换 | 小 |
| ℹ️ 低 | **模型拖拽排序** | 调整发言顺序 | 中 |
| ℹ️ 低 | **消息分页加载** | 长对话分页，优化性能 | 中 |
| ℹ️ 低 | **模型投票/评分** | 对回复打分记录偏好 | 中 |
| ℹ️ 低 | **多语言支持** | 界面国际化（i18n） | 中 |

#### 6.5.2 后端改进

| 优先级 | 建议 | 描述 | 工作量 |
|--------|------|------|--------|
| 🔴 高 | **真正的流式输出** | 改为实时 SSE 流（async generator，已完成） | 中 |
| ℹ️ 低 | **Anthropic Provider** | Claude 系列原生 API 支持（暂缓，暂无免费 API） | 中 |
| ⚠️ 中 | **请求重试机制** | 失败自动重试，指数退避（已完成） | 小 |
| ⚠️ 中 | **并行模型调用** | 多模型同时请求，缩短响应（已完成，默认关闭） | 中 |
| ⚠️ 中 | **API Key 加密存储** | 配置文件 key 加密（已完成） | 小 |
| ℹ️ 低 | **请求日志审计** | 记录 API 调用日志（已完成） | 小 |
| ℹ️ 低 | **WebSocket 替代 SSE** | 双向通信，支持取消 | 大 |
| ℹ️ 低 | **插件系统** | 自定义后处理器 | 大 |
...
| 优先级 | 建议 | 描述 | 收益 |
|--------|------|------|------|
| 🔴 高 | **SSE 事件生成器抽取** | `sessions.py` 的 `event_generator` 150+ 行，抽到独立模块（已完成） | 减少单文件行数 |
| ⚠️ 中 | **响应转换器迁移** | `_to_message_response` 等移到 `converters.py`（已完成） | 提高可维护性 |
| ⚠️ 中 | **重试逻辑模块化** | 创建 `backend/core/retry.py`（已完成） | 可复用 |
| ⚠️ 中 | **colorClasses 共享** | 前端 `chat.js` 和 `settings.js` 重复定义，移到 `utils.js` | 减少冗余 |
| ℹ️ 低 | **Google Provider 流式** | 实现 `generate_stream` 方法（已完成） | 真实流式支持 |
